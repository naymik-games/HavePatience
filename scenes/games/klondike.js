class playKlondike extends Phaser.Scene {
  constructor() {
    super("playKlondike");
  }
  preload() {
...
this.cardHeight = decks[gameOptions.deckNum].cardHeight * this.scale;
    ...
    this.discardPile = this.add.image((this.drawPile.x + this.cardWidth / 2) + this.cardSpacing + this.cardWidth / 2, 350, gameOptions.deck, this.cardBlank).setScale(this.scale).setOrigin(.5).setAlpha(.2);
    this.discardPile.type = 'discard';


    this.refreshIcon = this.add.image(this.edgeMargin + this.cardWidth / 2, 350, 'icons', 1);
  ...
  drawCard() {
    ...
    if (this.drawCount == 0 && this.refreshCount < 50) {
